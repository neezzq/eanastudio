...
export function processTextFill(src: HTMLCanvasElement, s: TextFillSettings): HTMLCanvasElement {
  ...
  const text = (s.text || 'HELLO WORLD ').replace(/\s+/g, ' ');
  ...
  for (let row = 0; row < totalLines; row++) {
    ...
    while (x < W) {
      ...
      const sampleX = Math.min(W - 1, Math.max(0, Math.round(x + avgCharW * 0.5)));
      const sampleY = Math.min(H - 1, Math.max(0, Math.round(y + fontSize * 0.5)));
      const sampleI = (sampleY * W + sampleX) * 4;
      const sampleAlpha = sd[sampleI + 3];
      const isOpaque = sampleAlpha > 20;
      const shouldDraw = s.fillOpaque ? isOpaque : !isOpaque;

      if (shouldDraw) {
        if (s.showOriginalColor && isOpaque) {
          const r = sd[sampleI], g = sd[sampleI + 1], b = sd[sampleI + 2];
